...
class KimiUser():
                ...
                messages=[
                    {"role": "system",
                     "content":
                         ...
                         "同时，你会拒绝一切涉及恐怖主义，种族歧视，黄色暴力等问题的回答。Moonshot AI 为专有名词，不可翻译成其他语言。"},
                    {"role": "user", "content":
                        f"请结合历史聊天信息（如存在），用户的简历打分+优化结果（包括岗位信息，"
                        f"用户简历内容，打分结果及建议等）（如存在），严格遵守给出的打分结果（如存在），回答用户说的话。"
    ...
    # def kimi_resume_optimization_chat_stream(self, history_chat_record: str, user_prompt: str, res_opt_record: str):
    #     """
    #     该方法主要作用是关于简历打分优化任务的聊天。
    #     修改为流式输出方法，返回一个生成器
    #     :param history_chat_record: 历史聊天记录
    #     :param user_prompt: 用户当前输入
    #     :param res_opt_record: 简历优化记录
    #     :return: 流式输出的生成器
    #     """
    #     log.info(history_chat_record)
    #     log.info(user_prompt)
    ...
    #                     f"请结合历史聊天信息（如存在），用户的简历打分+优化结果（包括岗位信息，"
    #                     f"用户简历内容，打分结果及建议等）（如存在），严格遵守给出的打分结果（如存在），回答用户说的话。"
    #                     f"如果用户说的话和简历优化无关，请向用户温和地表示自己是简历打分+优化助手，同时认真回复用户给出的内容。"
    #                     f"##历史聊天信息：{history_chat_record}。##用户的简历打分+优化结果：{res_opt_record}。##用户说的话："
    #                     f"{user_prompt}"},
    #             ],
    ...
    #                 yield content
    #
    #         log.info("kimi大模型聊天1任务流式输出完成")
    #
    #     except Exception as e:
    #         log.error(f"kimi大模型简历优化_聊天任务回答失败！原因：{e}")
    #         yield "kimi大模型聊天任务回答失败！"
...
if __name__ == '__main__':
    ...
    # 流式输出
    # for chunk in k.kimi_resume_optimization_chat("", "详细讲解一下计算机网络技术", "目标岗位：英语教师。简历结果：85分。"):
    #     print(chunk, end="", flush=True)  # end="" 防止自动换行
